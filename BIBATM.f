!C********************ATM********************************
      ...
      SUBROUTINE ATM(IATMOS,P,T,ALT,ROH2O,UH2O,ZSURF)

      INTEGER Q
      PARAMETER(NLEVEL=13,NLEV=10)
      DIMENSION P(NLEV),T(NLEV),ALT(NLEV),ROH2O(NLEV)
      DIMENSION P1(NLEVEL),T1(NLEVEL),ALT1(NLEVEL),ROH2O1(NLEVEL)
      DIMENSION ATMOS1(4,200),ATMOS2(4,200),GASES1(1,200)
...
!C      ppo    = gases1(2,74)
      DO 230 l=1,74
!c       i = 75 -l
!C      if(igases.eq.0) goto 210
!C      if(ppm.ne.ppo) gases1(2,74) = gases1(2,74)*(ppm/ppo)
!C      READ (10,200) (ATMOS1(j,i),j=1,4),
!C     *               gases1(1,i),(gases1(j,i),j=3,6)
      READ (10,200)(ATMOS1(j,l),j=1,4),GASES1(1,l)  !ONLY WATER CONTENT

  200 FORMAT (f7.2,e10.3,f7.1,e11.3,5e9.2)
      GOTO 230
!C  210 read (10,220) (ATMOS1(j,i),j=1,4)
!C  220 format (f7.2,e10.3,f7.1,e11.3)
  230 CONTINUE
      GOTO 270
  240 READ (10,250) 
  250 FORMAT (77(/))
...
!C ONLY NLEVEL
      DO Q=1,NLEVEL
	IF(Q.LE.(NLEVEL-2)) THEN
	 ALT1(Q)=ATMOS1(1,Q)
	 P1(Q)=ATMOS1(2,Q)
	 T1(Q)=ATMOS1(3,Q)
	 ROH2O1(Q)=GASES1(1,Q)
	ELSEIF (Q.EQ.(NLEVEL-1)) THEN !15 KM
	 ALT1(Q)=ATMOS1(1,16)
	 P1(Q)=ATMOS1(2,16)
	 T1(Q)=ATMOS1(3,16)
	 ROH2O1(Q)=GASES1(1,16)
	ELSEIF (Q.EQ.NLEVEL) THEN !120 KM
	 ALT1(Q)=ATMOS1(1,74)
	 P1(Q)=ATMOS1(2,74)
	 T1(Q)=ATMOS1(3,74)
	 ROH2O1(Q)=GASES1(1,74)
	ENDIF
      ENDDO
	


      IF(ZSURF.GT.3.9)ZSURF=3.9
      IF(ZSURF.LT.0.)ZSURF=0.0
      K=1
      DO J=1,4
       IF(ZSURF.GE.ALT1(J).AND.ZSURF.LT.ALT1(J+1))THEN
               K=J
               GOTO 1
       ENDIF
      ENDDO
 ...
 1    CONTINUE
      ALT(1)=ZSURF
      P(1)=P1(K)+(P1(K+1)-P1(K))/(ALT1(K+1)-ALT1(K))*(ZSURF-ALT1(K))
      T(1)=T1(K)+(T1(K+1)-T1(K))/(ALT1(K+1)-ALT1(K))*(ZSURF-ALT1(K))
      ROH2O(1)=ROH2O1(K)+(ROH2O1(K+1)-ROH2O1(K))/
     $(ALT1(K+1)-ALT1(K))*(ZSURF-ALT1(K))
      DO J=2,7
         P(J)=P1(J+K-1)
         T(J)=T1(J+K-1)
         ALT(J)=ALT1(J+K-1)
         ROH2O(J)=ROH2O1(J+K-1)
      ENDDO
      DO J=8,NLEV
         P(J)=P1(J+3)
         T(J)=T1(J+3)
      ...
      ENDDO
      DO J=1,NLEV
         ROH2O(J)=ROH2O(J)*1.0E-06*18./28.97
      ENDDO

      UH2O=0.
      DO L=1,NLEV-1
         UH2O=UH2O+(ROH2O(L)+ROH2O(L+1))/2.*(P(L)-P(L+1))/.981
      ENDDO
...
!C************************ATMPROF****************************
      ...
      SUBROUTINE ATMPROF(IMOLABS,LAMB,RAYL,UD,RAY,RKLM,IK1,IK2,MOLSCT,
     ...
     &                   XABSCO2,XABSO2,XABSO3,IRESOL,ICNTCO2)
      ...
      DIMENSION RAY(MAXULV),RKLM(NKDMAX,MAXCLY,NBABS),PRES(MAXULV),
     &          UD(NBABS,MAXULV),TT(MAXULV),ALTIT(MAXULV),
     &          PMOMA(MAXCMU,MAXCLY),AERTAU(MAXCLY),AERP(MAXCLY),
     &          DTAUC(MAXCLY),SSALB(MAXCLY),PMOM(0:MAXCMU,MAXCLY)
...
!C-----------------------------
      IF(MOLSCT.EQ.1)THEN
          ...
          p1=pres(j)*PS
          p2=pres(j+1)*PS
          call rayleigh(p1,p2,anu,tauray)
          tem=tauray
          TSM=TEM
          BETAL=0.5
      ELSE
...
!C-----------------------------
      DPRES=(PRES(J)-PRES(J+1))*PS
      PR=(PRES(J)+PRES(J+1))/(2.)*PS
      TP=(TT(J)+TT(J+1))/2.
      ALT1=ALTIT(J)
      ALT2=ALTIT(J+1)

      TMOLH2O=0.
      IF(XABSH2O.NE.0)THEN

       IF(IMOLABS.EQ.1.AND.ICNTWV.NE.1)THEN
         TMOLH2O=RKLM(IK1,J,1)*UD(1,J)
       ELSE
         TMOLH2O=0.
       ENDIF
!C------- WATER VAPOR CONTINUUM ---------------------
       IF(ICNTWV.GE.1.AND.XABSH2O.GT.0.AND.ANU.LT.20000)THEN
         ROH2O=UD(1,J)
         WV=ANU
         CALL CONTH2O(WV,ROH2O,DPRES,PR,TP,ALT1,ALT2,TAUCNTWV)
         TMOLH2O=TMOLH2O+TAUCNTWV
       ENDIF
      ENDIF

      TMOLCO2=0.
      IF(XABSCO2.NE.0.OR.XABSO2.NE.0)THEN
       UDCO2=0
       IF(IMOLABS.EQ.1.AND.ICNTCO2.NE.1)THEN
        IF((ANU.GT.11000).OR.(ANU.GT.7800.AND.ANU.LT.8000.))THEN
             UDCO2=UD(2,J)*PPMO2*COEFO2*XABSO2
        ELSE
             UDCO2=UD(2,J)*PPMCO2*COEFCO2*XABSCO2
        ENDIF
        TMOLCO2=RKLM(IK2,J,2)*UDCO2
       ELSE
        TMOLCO2=0.
       ENDIF
!C------- CARBON DIOXIDE CONTINUUM ---------------------
       IF(ICNTCO2.GE.1.AND.XABSCO2.GT.0.AND.ANU.LT.10000)THEN
         PR=(PRES(J)+PRES(J+1))/(2.*1013.25)*PS
         TP=296./((TT(J)+TT(J+1))/2.)
         ROCO2=UD(2,J)*PPMCO2*COEFCO2*XABSCO2
         CALL CNTCO2(ANU,ROCO2,TAUCNTCO2,PR,TP,IRESOL)
         TMOLCO2=TMOLCO2+TAUCNTCO2
       ENDIF
      ENDIF

      TMOL=TMOLH2O+TMOLCO2

!C------- OZONE CONTINUUM ---------------------
      TMOLO3=0.
      IF(UD(3,J).GT.0.AND.(ANU.GT.24565.OR.(ANU.GT.9170.AND.
     $   ANU.LT.24565.)))THEN
         ROO3=UD(3,J)
         TEMP=(TT(J)+TT(J+1))/2.
         CALL CNTO3(ANU,ROO3,TEMP,TAUCNTO3,IRESOL)
         TMOLO3=TMOLO3+TAUCNTO3
      ENDIF
      TMOL=TMOLH2O+TMOLCO2+TMOLO3
!C
!C----------------------------------------------
!C     ATMOSPHERIC PROFILE: DTAUC + SSALB + PMOM
!C----------------------------------------------
      TST=TSM+TSA
      TET=TMOL+TEM+TEA
      DTAUC(L)=TET
...
!C
      ...
      IF(TST.EQ.0.)GOTO 6
      PMMOL=0.
      IF(MOLSCT.EQ.1.AND.K.EQ.2) PMMOL=BETAL/5.
      IF(AERO.NE.0.)THEN
           PMAER=PMOMA(K,J)
      ELSE
           PMAER=0.
      ...
      PMOM(K,L)=(TSA*PMAER+TSM*PMMOL)/TST
 6    CONTINUE                                                                  
 5    CONTINUE
 1    CONTINUE                                                                  
      RETURN
...
!C******************END ATMPROF*********************************
!C*****************CHECK_NSTR***********************************
      SUBROUTINE CHECK_NSTR(NSTR,UMU0,MAXCMU)
      DIMENSION     CMU(MAXCMU), CWT(MAXCMU)
      LOGICAL TEST
 ...
 2    NN = NSTR / 2
      ...
      DO IQ = 1, NN
       ...
       ENDIF
      ENDDO
 1    IF(.NOT.TEST.AND.IPASS.LT.4)GOTO 2
...
!C                                                                         F02570
      COMMON /SH2O1/ V1,V2,DV,NPT,                                        F02580
...
!C                                                                         F07520
      COMMON /S2601/ V1,V2,DV,NPT,                                         F07530
...
!C                                                                         F12490
      COMMON /FH2O1/ V1,V2,DV,NPT,                                         F12500
...
!C*****************END CNTDATA***********************************************
!c*****************VARSOL1*************************************************
      subroutine varsol1 (jday,month,dsol)
...
!C******************END VARSOL*************************************************
!c*********************KD******************************************************
      SUBROUTINE KD(NKDMAX,MAXCLY,MAXULV,NKDMOL,NLEVEL,NBTEMP,
     $              NBABS,NBINTABS,ICNT,
     $              LAMB,UD,PRES,TT,AL,RKLM,UMU0,XABS,IMOL,SA,SOI,
     $              Y1,TPAR,X1,GKL,RKL,ALDP,H,RKLMDP,TMP,UDCO2)
!C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION GKL(0:NKDMAX),RKL(0:NKDMAX),
     $          ALDP(NKDMAX),H(0:NKDMAX),RKLMDP(NKDMAX),TMP(NBTEMP)
      REAL UD(NBABS,MAXULV),PRES(MAXULV),TT(MAXULV),UMU0,
     $     SOI(NBTEMP,NBINTABS,NBABS),SA(NBTEMP,NBINTABS,NBABS),
     $     AL(NKDMAX,NBABS),RKLM(NKDMAX,MAXCLY,NBABS),XABS,UDCO2
      REAL Y1(NBTEMP),TPAR(NBTEMP),X,YP1,YPN,YY,X1(NBTEMP)
!C
!C-------------------------------------------------------------
!C
!C---------------------------------------------------------
...
!C-----   TRANSMISSION = 1.0     ----
!C
      IF(LAMB.GT.NBINTABS.OR.ICNT.EQ.1)THEN
         DO K=1,NKD
           AL(K,IMOL)=1.0
           DO J=1,NLEVEL-1
              RKLM(K,J,IMOL)=0.0
...
!C
!C--------------------------------------------------------------------
!C ----   CALCUL DES TEMPERATURE ET PRESSION MOYENNES DE LA COUCHE ---
!C--------------------------------------------------------------------
!C
      TM=(TT(L)+TT(L+1))/2
      PM=(PRES(L)+PRES(L+1))/2
      IF(TM.LT.190.)TM=190.
      IF(TM.GT.320.)TM=320.
!C
!C-------------------------------------------------------------
!C ----   INTERPOLATION DES PARAMETRES SPECTROSCOPIQUES   ----
!C ----         AUX VALEURS MOYENNES PRECEDENTES          ----
!C-------------------------------------------------------------
!C
      YP1=(SOI(2,LAMB,IMOL)-SOI(1,LAMB,IMOL))/(TMP(2)-TMP(1))
      YPN=(SOI(NBTEMP,LAMB,IMOL)-SOI(NBTEMP-1,LAMB,IMOL))/(TMP(NBTEMP)
     $    -TMP(NBTEMP-1))
      DO I=1,NBTEMP
       Y1(I)=SOI(I,LAMB,IMOL)
       X1(I)=TMP(I)
      ENDDO
      CALL SPLINE1(X1,Y1,NBTEMP,YP1,YPN,TPAR)
      X=TM
      CALL SPLINT1(X1,Y1,TPAR,NBTEMP,X,YY)
      SIM=YY

      YP1=(SA(2,LAMB,IMOL)-SA(1,LAMB,IMOL))/(TMP(2)-TMP(1))
      YPN=(SA(NBTEMP,LAMB,IMOL)-SA(NBTEMP-1,LAMB,IMOL))/(TMP(NBTEMP)
     $    -TMP(NBTEMP-1))
      DO I=1,NBTEMP
       Y1(I)=SA(I,LAMB,IMOL)
      ENDDO
      ...
      SAM=YY
!C-------------------------------------------------------------
!C ----   EFFET DE LA PRESSION SUR SA   ----
!C
      SAM=SAM*PM
!C
!C-------------------------------------------------------------
!C ----   PARAMETRES DE L'EXPRESSION DE MALKMUS   -----
!C
      B=SAM
      S=SIM
!C
...
!C-----   TRANSMISSION = 1.0     ----
!C
      IF((XABS.EQ.0).OR.(S.EQ.0.AND.L.EQ.1))THEN
         DO K=1,NKD
           AL(K,IMOL)=1.0
           DO J=1,NLEVEL-1
              RKLM(K,J,IMOL)=0.0
...
!C 
!C-------------------------------------------------------------
!C ---- RESOLUTION DE L'EQUATION GKL(I)=GK(RKL) -----
!C ----       PAR LA METHODE DE NEWTON          -----
!C ----   EN SORTIE LES VALEURS DE KL (RKL)     -----
!C ----  ET LES VALEURS DE ALDP=GKL(I)-GKL(I-1)   -----
!C-------------------------------------------------------------
!C
      RKMAX=3*S/B*(DSQRT((B/3)*(B/3)+1)-1)
      DO 4 I=1,NKD
        CALL NEWTRAP(RKMAX,B,S,GKL(I),RKL(I),INEWT)
        ALDP(I)=GKL(I)-GKL(I-1)
 4    CONTINUE
      IF(INEWT.EQ.1)THEN
!C        WRITE(6,*)L,RKMAX,S,B
         DO K=1,NKD
           RKLMDP(K)=0.
         ENDDO
         GOTO 1
...
!C
!C-------------------------------------------------------------
!C ----   CALCUL DES KL MOYENS POUR CHAQUE COUCHE  ----
!C
      IF(UMU0.LT..7)THEN
             UMUM=.7
                    ELSE
      ...
      ENDIF
      UMOY=UD(IMOL,L)/UMUM
      IF(IMOL.EQ.2)UMOY=UMOY*UDCO2
      TM=DEXP(-B/2*(DSQRT(1+4*UMOY*S/B)-1))
      TMI=DEXP(B/2*(DSQRT(1+4*UMOY*S/B)-1))
      H(0)=TOL
      DO 6 I=1,NKD
         R3=DSQRT(B*S/4/RKL(I))
         R4=DSQRT((B/S+4*UMOY)*RKL(I)/4)
         R44=-R4
         H(I)=.5*(DEXP(B)*TMI*ERFCGAME(R3,R4)+TM*ERFCGAME(R3,R44))
         DIH=H(I)-H(I-1)
         IF(DIH.LT.1E-30)DIH=1E-30
         RAP=DIH/ALDP(I)
         IF(RAP.GE.1.0)RAP=0.99999999
         RKLMDP(I)=-1/UMOY*DLOG(RAP)
 6    CONTINUE
!C
!C-------------------------------------------------------------
!C     RESULTATS EN SIMPLE PRECISION
!C
 1    DO I=1,NKD
        RKLM(I,L,IMOL)=RKLMDP(I)
        AL(I,IMOL)=ALDP(I)
      ENDDO
      NKDMOL=NKD
...
!C**************************RAYLEIGH************************************
      ...
      subroutine rayleigh(p1,p2,wvn,taurayfro)

      data p0 /1013.25/
      data A,B,C,D/0.00838,3.916,0.074,0.050/

      wv=10000./wvn
      beta=1.031*A*wv**(-(B+C*wv+D/wv))
      taurayfro1=beta*p1/p0
      taurayfro2=beta*p2/p0
!C optical thickness
      taurayfro=taurayfro1-taurayfro2
...
!c************************CNTO3*********************************************
      SUBROUTINE CNTO3(ANU,ROO3,TEMP,TAUCNTO3,IRESOL)
     
      COMMON /O3HUV/  V1S,V2S,DVS,NPTS,S(133)                           
      COMMON /O3CHAP/ V1O1,V2O1,DVO1,NPT1,O3CH(55)                    
      COMMON /O3HH01/ V10S,V20S,DV0S,NPTS0,S0(2687)
      COMMON /O3HH11/ V11S,V21S,DV1S,NPTS1,S1(2690)
      COMMON /O3HH22/ V12S,V22S,DV2S,NPTS2,S2(2690)

!C--------------------------------------------------------------------
!C                     PARAMETRES D'ENTREE
!C--------------------------------------------------------------------
!C  ROO3 ---------> QUANTITE D'ABSORBANT DE LA COUCHE EN CM/ATM
!C  TEMP    ---------> TEMPERATURE DE LA COUCHE
!C  ANU ---------> FREQUENCE DE CALCUL
!C--------------------------------------------------------------------

!C--------------------------------------------------------------------
!C   CALCUL DE L'ABSORPTION PAR LE CONTINUUM DE L'OZONE
!C--------------------------------------------------------------------
!C--------- CONTINUUM DE LA BANDE D'ABSORPTION DE CHAPPUIS -----------

      TAUO3CHAP=0.0
      TC=TEMP-273.15
      IF(ANU.LT.9170.0.OR.ANU.GE.24565.0)GOTO 1
      IF(IRESOL.EQ.1)THEN
          IF(ANU.LT.14400.)THEN
            I1CH=INT((ANU-50-9170.0)/5)+1
            NBCH=19
            DELTANU=50
          ELSE
            I1CH=INT((ANU-200-9170.0)/5)+1
            NBCH=79
            DELTANU=200
          ENDIF
      ELSE
          I1CH=INT((ANU-5-9170.0)/5)+1
          NBCH=1
          DELTANU=5
      ENDIF

      TRSMOY=0.
      DO ICH=I1CH,I1CH+NBCH
        V=ANU-DELTANU+5*(ICH-I1CH)
        CALL O3CHAPW(V,C0,CT1,CT2)
        COEFEXTCH=(C0+CT1*TC+CT2*TC*TC)*1.0E-20
        TRSMOY=TRSMOY+EXP(-COEFEXTCH*ROO3*2.6867E+19)
      ENDDO
      TRSMOY=TRSMOY/(NBCH+1)
      IF(TRSMOY.LT.1.0E-30)TRSMOY=1.0E-30 !CXXA
      COEFEXTCH=-ALOG(TRSMOY)/ROO3/2.6867E+19
      TAUO3CHAP=COEFEXTCH*ROO3*2.6867E+19
 1    CONTINUE

!C--------- CONTINUUM DE HARTLEY HUGGINS DANS L'UV -------------------

      TAUO3UV=0.0
      IF(IRESOL.EQ.1)THEN
         IF(ANU.LE.40600.0)GOTO 2
         IUV=INT((ANU-200-40800.0)/100)+1
         COEFEXTUV=S(IUV)+S(IUV+1)+S(IUV+2)
         TAUO3UV=COEFEXTUV/4*ROO3*2.6867E+19
      ELSE
         IF(ANU.LE.40800.0)GOTO 2
         IUV=INT((ANU-40800.0)/100)+1
         COEFEXTUV=(S(IUV+1)-S(IUV))/100*(ANU-(40800.0+100*(IUV-1)))+
     $             S(IUV)
         TAUO3UV=COEFEXTUV*ROO3*2.6867E+19
      ENDIF
 2    CONTINUE
...
!C
!C     TAUO3CHAP=0.0
!C     IF((IRESOL.EQ.2.AND.(ANU.GT.12800.AND.ANU.LE.23600))
!C    $.OR.(IRESOL.EQ.1.AND.(ANU.LT.14400.AND.ANU.GE.12800)))THEN
!C       I1CH=INT((ANU-12800.0)/200)+1
!C       COEFEXTCH=(O3CH(I1CH+1)-O3CH(I1CH))/200.0*(ANU-(12800.0+
!C    $             200.0*(I1CH-1)))+O3CH(I1CH)
!C     TAUO3CHAP=COEFEXTCH*ROO3
!C     ENDIF
!C     IF(IRESOL.EQ.1.AND.(ANU.LT.23600.AND.ANU.GT.14400))THEN
!C       COEFEXTCH=0.
!C       I1CH=INT((ANU-200-12800.0)/200)+1
!C       COEFEXTCH=(O3CH(I1CH)+O3CH(I1CH+1))/2
!C     TAUO3CHAP=COEFEXTCH*ROO3
!C     ENDIF
!C2    CONTINUE

!C--------- CONTINUUM DE HARTLEY HUGGINS -----------------------------

      TAUO3HH=0.0
      TC=TEMP-273.15
      IF(IRESOL.EQ.1)THEN
          IF(ANU.LT.27570.0.OR.ANU.GE.40800.0)GOTO 3
          I1HH=INT((ANU-200-27370.0)/5)+1
          NBHH=79
      ELSE
          IF(ANU.LT.27370.0.OR.ANU.GE.40800.0)GOTO 3
          I1HH=INT((ANU-5-27370.0)/5)+1
          NBHH=1
      ENDIF
      TRSMOY=0.
      DO IHH=I1HH,I1HH+NBHH
        COEFEXTHH=S0(IHH)*(1.+TC*S1(IHH)+TC*TC*S2(IHH))*1.0E-20
        TRSMOY=TRSMOY+EXP(-COEFEXTHH*ROO3*2.6867E+19)
      ENDDO
      TRSMOY=TRSMOY/(NBHH+1)
      IF(TRSMOY.LT.1.0E-30)TRSMOY=1.0E-30
      COEFEXTHH=-ALOG(TRSMOY)/ROO3/2.6867E+19
      TAUO3HH=COEFEXTHH*ROO3*2.6867E+19
 3    CONTINUE

      END                                                               

      BLOCK DATA O3CH                                                   
      COMMON /O3CHAP/ V1O1,V2O1,DVO1,NPT1,O3DIF1(55)                    
!C                                                                       
!C     REFERENCE : LOWTRAN 7
!C     BANDE D'ABSORPTIOON DE CHAPPUIS
!C     VALEURS DES COEFFICIENTS ENTRE 12800. ET 23600. CM-1 
!C        PAR PAS DE 200 CM-1
!C     UNITES : (CM-ATM)-1
!C     MODIFICATION (13/04/94) 55 VALEURS AU LIEU DE 54 (0. A 12800)
!C     
!C                                                                       
      DATA V1O1,V2O1,DVO1,NPT1/ 12800.,23600.,200.,55/                  
!C                                                                       
      DATA O3DIF1 / 0.,                                                 
...
!C                                                                       
!C     REFERENCES : INN ET TANAKA, J.OPT.SOC.,43,870-873,1953
!C                  MOLINA ET MOLINA, JGR,91,14501-14508,1986 
!C     VALEURS DES COEFFICIENTS ENTRE 27370. ET 40800. CM-1
!C        PAR PAS DE 5 CM-1 AVEC UNE DEPENDANCE EN TEMPERATURE
!C        DU TYPE C(T)=C0+C1*T+C2*T OU C(T)=S0*(1+S1(T)*T+S2(T)*T*T)
!C        S0 ---> BO3HH0  C0:COEFFICIENTS A 273.15 K
!C        S1 ---> BO3HH1  C1/C0
!C        S2 ---> BO3HH2  C2/C0
!C     UNITES : (CM**2/MOLEC)*1.0E-20
!C                                                                       
...
!C                                                                       
      ...
      BLOCK DATA BO3HH11                                                 
!C                                                                       
!C  VALEURS DES COEFFICIENTS C1/C0
!C                                                                       
      COMMON /O3HH11/  V1C,V2C,DVC,NC,                                   
...
!C                                                                       
     ...
     X-2.30905E-04,-2.11669E-04,-2.37680E-04,-2.38194E-04,-2.10955E-04/ 
      ...
      BLOCK DATA BO3HH22                                                 
!C                                                                       
!C  VALEURS DES COEFFICIENTS C2/C0
!C                                                                       
      COMMON /O3HH22/  V1C,V2C,DVC,NC,                                   
...
!C                                                                       
     ...
     X-1.51037E-06,-1.43610E-06,-2.11316E-06,-2.45184E-06,-2.42262E-06/ 
      ...
      SUBROUTINE O3CHAPW(V,C0,CT1,CT2)
!C
...
!C***********************END INTERPOL**********************************
!c************************CONTCO2*************************************
      SUBROUTINE CNTCO2(FREQ,ROCO2,XCNTCO2,PR,TP,IRESOL)

      DOUBLE PRECISION TRSMOY
      COMMON /FCO2/ V1S,V2S,DVS,NPTS,S(1003)

      ROCO2=ROCO2*9.81/10*1.395E+22
      IF(IRESOL.EQ.2)THEN
         ICT=INT(FREQ/10.)+3
         CCO2=S(ICT)
         XCNTCO2=CCO2*1.0E-20*PR*TP*ROCO2*FREQ*1.0005
      ELSE
         ANU=FREQ-50
         NBINT=9
         TRSMOY=0.
         DO ICONT=1,NBINT+1
           ...
           ICT=INT(WV/10.)+3
           CCO2=S(ICT)
           XCNTCO2=CCO2*1.0E-20*PR*TP*ROCO2*WV*1.0005
           TRSMOY=TRSMOY+EXP(-XCNTCO2)
         ENDDO
         TRSMOY=TRSMOY/(NBINT+1)
         IF(TRSMOY.LT.1.0E-08)TRSMOY=1.0E-08
         XCNTCO2=-DLOG(TRSMOY)
     X 1.1086E-17, 1.2686E-17, 1.4679E-17, 1.7177E-17, 2.0335E-17,
     X 2.4384E-17, 2.9538E-17, 3.6416E-17, 4.5520E-17, 5.7788E-17,
     X 7.4676E-17, 9.8513E-17, 1.3323E-16, 1.8570E-16, 2.6897E-16,
     X 4.0958E-16, 6.6785E-16, 1.2064E-15, 2.4023E-15, 4.3240E-15,
     X 6.6353E-15, 8.6393E-15, 1.1433E-14, 1.3946E-14, 1.3611E-14,
     X 1.2557E-14, 1.0934E-14, 1.0039E-14, 8.5099E-15, 7.9557E-15,
     X 1.1346E-14, 1.8512E-14, 2.9285E-14, 4.1585E-14, 5.2809E-14/
      DATA C0951/
     X 7.0377E-14, 7.8094E-14, 7.3735E-14, 6.5845E-14, 5.5023E-14,
     X 4.6866E-14, 2.7430E-14, 1.5975E-14, 1.4522E-14, 1.7075E-14,
     X 2.0408E-14, 2.5119E-14, 3.1194E-14, 3.0280E-14, 2.7676E-14,
     X 2.3344E-14, 1.9466E-14, 1.4140E-14, 6.2087E-15, 3.0307E-15,
     X 1.6815E-15, 1.0169E-15, 6.5448E-16, 4.4162E-16, 3.0928E-16,
     X 2.2320E-16, 1.6511E-16, 1.2471E-16, 9.5881E-17, 7.4850E-17,
     X 5.9216E-17, 4.7400E-17, 3.8338E-17, 3.1298E-17, 2.5765E-17,
     X 2.1371E-17, 1.7848E-17, 1.5000E-17, 1.2679E-17, 1.0774E-17,
     X 9.2002E-18, 7.8922E-18, 6.7987E-18, 5.8800E-18, 5.1042E-18,
     X 4.4461E-18, 3.8855E-18, 3.4060E-18, 2.9944E-18, 2.6397E-18/
      DATA C1001/
     A 2.3331E-18/
!C
      END
!C*************************END_CONTCO2*****************************
!C**************************CONTH2O********************************
      SUBROUTINE CONTH(FREQ,CS0,CS,CF)

      DOUBLE PRECISION FREQ

      COMMON /SH2O1/ V1S1,V2S1,DVS1,NPTS1,SH2OT0(2003)
      COMMON /S2601  / CV1S2,V2S2,DVS2,NPTS2,SH2OT1(2003)
      COMMON /FH2O1/ CV1S3,V2S3,DVS3,NPTS3,FH2O(2003)
!C********    WATER    ********                                          
!C                                                                       
!C                                                                       
      ICONT=INT(FREQ/10.)+3
...
!C
      ...
      SUBROUTINE CONTH2O(WV,ROH2O,DPRES,PR,TP,ALT1,ALT2,TAUH2O)
...
!C TP: K
     ...
     9    1.00000,1.00000,1.00000/
      ...
      DATA ATMOCM1,PREF,TREF,XLOSMT/3.410E+22,1013.25,296.0,2.68675E+19/

      ROH2O=ROH2O*9.81/10*28.97/18.0/DPRES

      TAUH2O=0.
      IF(WV.GE.20000)GOTO 1
      WTOT=XLOSMT*273.15/TP*PR/PREF*(ALT2-ALT1)*1.0E+05
      WH2O=ROH2O*WTOT/(1+ROH2O)
      ROH2OS=TREF/TP*PR/PREF*WH2O/WTOT
      ROH2OF=TREF/TP*PR/PREF*(1-WH2O/WTOT)

      IF(WV.GT.14400)NBINT=39
      IF(WV.LT.14400)NBINT=9
      IF(WV.GT.14400)FREQ=WV-200
      IF(WV.LT.14400)FREQ=WV-50
      TRSMOY=0.
      DO ICT=1,NBINT+1
         WV=FREQ+10.*(ICT-1)

      CALL CONTH(WV,CS0,CS,CF)
      VJ=WV
      TFAC = (TP-TREF)/(260.-TREF)
!C
!C--------------------------------------------------------------------
!C                             SELF NEW
      V0S1 = 0.
...
!C
          VF2 = (VJ-V0F2)**2
          VF6 = VF2 * VF2 * VF2
          FSCAL = FSCAL*(1.+FACTRF2*(HWSQF2/(VF2+(BETAF2*VF6)+HWSQF2)))
!C
          VF2 = (VJ-V0F3)**2
          VF4 = VF2*VF2
          FSCAL = FSCAL*(1.+FACTRF3*(HWSQF3/(VF2+BETAF3*VF4+HWSQF3)))
         FH2O=CF*FSCAL
!c       write(6,*)'FOREIGN NEW=',FH2O

!C SELF + FOREIGN:
!C
       CNTWV=(SH2O*ROH2OS+FH2O*ROH2OF)*1.0E-20
       VKT=WV*6.626176E-27*2.99792458E10/1.380662E-16/TP
       CNTWV=CNTWV*WV*(1.-EXP(-VKT))/(1.+EXP(-VKT))
       TAUH2O=CNTWV*ROH2O*WTOT/(1+ROH2O)

!C--------------------------------------------------------------------
      TRSMOY=TRSMOY+EXP(-TAUH2O)
      ENDDO
...
!c**************************NEWTON*****************************************
      ...
      P=.3275911
      T=1/(1+P*X)
      ERF=1-(.254829592*T-.284496736*T*T+1.421413741*T*T*T-
     $    1.453152027*T*T*T*T+1.061405429*T*T*T*T*T)*DEXP(-X*X)
 1    IF(X1.LT.0.)ERF=-ERF
      ERFCGAME=1-ERF
      END

!C------------------------------------------------------------

      SUBROUTINE NEWTRAP(RKMAX,B,S,GKL,RKL,INEWT)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      PARAMETER(NTRIAL=50,NP=15)
      DIMENSION X(NP),ALPHA(NP,NP),BETA(NP)
      X(1)=RKMAX
!C
!C     tolx, toly = seuils de tolerance :
!C     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!C     'variation min acceptee pour la racine :'
!C     'tolerance sur la fonction :'
      TOLF=1D-9
      TOLX=1D-9
      INEWT=0
!C     WRITE(*,*)'  pt de depart : ',X(1)
!C
!C     ntrial = nb max d'iterations :
!C     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      DO 13 J=1,NTRIAL
      CALL MNEWT(1,X,S,B,GKL,TOLX,TOLF,ALPHA,BETA)
      CALL USRFUN(X,S,B,GKL,ALPHA,BETA)
      if(dabs(beta(1)).lt.tolf) goto 14
13    CONTINUE
!c     write(6,*)'la methode de newton ne converge pas'
      INEWT=1
14    continue
      RKL=X(1)
      RETURN
      END
!C================================================
      SUBROUTINE USRFUN(X,S,B,GKL,ALPHA,BETA)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      PARAMETER(NP=15)
      ...
      R2=DSQRT(B*RK/4/S)
      R22=-R2
      FN=.5*DEXP(B)*ERFCGAME(R1,R2)+.5*ERFCGAME(R1,R22)-GKL
      FND=.5*RK**(-1.5)*DSQRT(B*S)*DEXP(B/4*(2-S/RK-RK/S))
      beta(1)=-FN
      alpha(1,1)=FND
      return
      END
!C================================================
      SUBROUTINE MNEWT(NTRIAL,X,S,B,GKL,TOLX,TOLF,ALPHA,BETA)
      PARAMETER (NP=15)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION X(NP),ALPHA(NP,NP),BETA(NP)
      SAVE
      DO 13  K=1,NTRIAL
        CALL USRFUN(X,S,B,GKL,ALPHA,BETA)
        ERRF=0.
          ERRF=ERRF+ABS(BETA(1))
        IF(ERRF.LE.TOLF)RETURN
        Beta(1)=beta(1)/alpha(1,1)
        ERRX=0.
          ERRX=ERRX+ABS(BETA(1))
          X(1)=X(1)+BETA(1)
...
!C*********************END_NEWTON************************************
!c**********************O3ABS***************************************
      SUBROUTINE O3ABS(PADO3,TO3INF,UM,NUMU,NPHI,UMU,UD,NLEVEL,LAMB,
     $                 MAXULV,MAXUMU,MAXPHI,NBNUMAX,NBABS,FLXNU,RADNU,
     $                 DIRRADNU,FLXSPHNU)
      DIMENSION PADO3(2,4,105:208),TO3INF(105:208),UM(MAXULV,0:MAXUMU),
     $          UD(NBABS,MAXULV),UMU( MAXUMU ),FLXNU(MAXULV,NBNUMAX,2),
     $          RADNU(MAXUMU,MAXULV,MAXPHI,NBNUMAX),
     $          DIRRADNU(MAXUMU,MAXULV),FLXSPHNU(MAXULV,NBNUMAX)
       ...
       TDDOWN=TDUP
        ...
        ENDIF
          DO  KP=2 , 4
             ...
             TDUP=TDUP+RDC*PUISS
             PUISS=UDES**(KP-1)
             TNDOWN=TNDOWN+RNC*PUISS
             TDDOWN=TDDOWN+RDC*PUISS
             IF( RDC.EQ. 1 )GOTO  66
          ENDDO
   66     CONTINUE
          IF(I1.EQ.0)THEN
                  TUP3=TINF+(1-TINF)*TNUP/TDUP
                  TDOWN3=TINF+(1-TINF)*TNDOWN/TDDOWN
                  FLXNU(J,LAMB,2)=FLXNU(J,LAMB,2)*TUP3
                  FLXNU(J,LAMB,1)=FLXNU(J,LAMB,1)*TDOWN3
                  FLXSPHNU(J,LAMB)=FLXSPHNU(J,LAMB)*TDOWN3
                     ELSE
                  DO I2=1,NPHI
                  IF(UMU(I1).GT.0.)THEN
                      TUP3=TINF+(1-TINF)*TNUP/TDUP
                      RADNU(I1,J,I2,LAMB)=RADNU(I1,J,I2,LAMB)*TUP3
                      DIRRADNU(I1,J)=DIRRADNU(I1,J)*TUP3
                           ELSE
                      TDOWN3=TINF+(1-TINF)*TNDOWN/TDDOWN
                      RADNU(I1,J,I2,LAMB)=RADNU(I1,J,I2,LAMB)*TDOWN3
                  ENDIF
                  ENDDO
          ENDIF
         ENDDO
...
!C**********************END_O3ABS************************************
!C*********************O3PADE****************************************
      SUBROUTINE O3PADE(PADO3,TO3INF)
!C
      DIMENSION PADO3(2,4,105:208),TO3INF(105:208)
      DIMENSION TPAD(2,4,105:208),TINF(105:208)
!C
      DATA (((TPAD(I,J,INU),J=1,4),I=1,2),TINF(INU),
     &     INU=105,112) /
     & .1041037E+06, .1007260E+04, .0000000E+00, .0000000E+00,
     & .1041037E+06, .1124370E+04, .1000000E+01, .0000000E+00, .90,
     & .1101654E+06,-.1076500E+03, .0000000E+00, .0000000E+00,
     & .1101654E+06, .4844900E+03, .1000000E+01, .0000000E+00, .00,
     & .7745380E+05,-.1343700E+03, .0000000E+00, .0000000E+00,
     & .7745380E+05, .4174900E+03, .1000000E+01, .0000000E+00, .00,
     & .1485850E+06, .5568360E+04, .1000000E+01, .0000000E+00, .00/
      DATA (((TPAD(I,J,INU),J=1,4),I=1,2),TINF(INU),
     &     INU=153,160) /
     & .3017391E+06, .4787500E+03, .0000000E+00, .0000000E+00,
     & .3017391E+06, .9162700E+03, .1000000E+01, .0000000E+00, .00,
     & .2239681E+06,-.1869300E+03, .0000000E+00, .0000000E+00,
     & .2239681E+06, .7456700E+03, .1000000E+01, .0000000E+00, .00,
     & .5184779E+05,-.1449500E+03, .0000000E+00, .0000000E+00,
     & .5184779E+05, .3803200E+03, .1000000E+01, .0000000E+00, .00,
     & .1145747E+05,-.7372000E+02, .0000000E+00, .0000000E+00,
     & .1145747E+05, .1778300E+03, .1000000E+01, .0000000E+00, .00,
     & .2691120E+04,-.3600000E+02, .0000000E+00, .0000000E+00,
     & .2691120E+04, .8474000E+02, .1000000E+01, .0000000E+00, .00,
     & .6133900E+03,-.1597000E+02, .0000000E+00, .0000000E+00,
     & .6133900E+03, .3968000E+02, .1000000E+01, .0000000E+00, .00,
     & .9026530E+04,-.5166200E+03, .9150000E+01, .0000000E+00,
     & .9026530E+04, .1010670E+04, .4701000E+02, .1000000E+01, .00,
     & .1319340E+04,-.1259200E+03, .3580000E+01, .0000000E+00,
     & .1319340E+04, .2751600E+03, .2405000E+02, .1000000E+01, .00/
      DATA (((TPAD(I,J,INU),J=1,4),I=1,2),TINF(INU),
     &     INU=161,168) /
     & .1855300E+03,-.2973000E+02, .1310000E+01, .0000000E+00,
     & .1855300E+03, .7064000E+02, .1161000E+02, .1000000E+01, .00,
     & .1396070E+02,-.2906700E+01, .1536780E+00, .0000000E+00,
     & .1396070E+02, .1008090E+02, .3128650E+01, .1000000E+01, .00,
     & .3893510E+01,-.1508290E+01, .1512620E+00, .0000000E+00,
     & .3893510E+01, .4625720E+01, .2481650E+01, .1000000E+01, .00,
     & .4347440E+00,-.2223980E+00, .2880080E-01, .0000000E+00,
     & .4347440E+00, .9454260E+00, .8577220E+00, .1000000E+01, .00,
     & .1296280E+00,-.1322040E+00, .3556610E-01, .0000000E+00,
     & .1296280E+00, .4365720E+00, .6435200E+00, .1000000E+01, .00,
     & .4636750E-01,-.7968420E-01, .3464680E-01, .0000000E+00,
     & .4636750E-01, .2420660E+00, .5680210E+00, .1000000E+01, .00,
     & .9815000E-02,-.2739160E-01, .1919580E-01, .0000000E+00,
     & .9815000E-02, .8397860E-01, .3225320E+00, .1000000E+01, .00,
     & .2338750E-02,-.1043410E-01, .1165800E-01, .0000000E+00,
     & .2338750E-02, .3204040E-01, .1975250E+00, .1000000E+01, .00/
      DATA (((TPAD(I,J,INU),J=1,4),I=1,2),TINF(INU),
     &     INU=169,176) /
     & .6729260E-03,-.4673280E-02, .8173930E-02, .0000000E+00,
     & .6729260E-03, .1408890E-01, .1328340E+00, .1000000E+01, .00,
     & .2213350E-03,-.2352730E-02, .6363620E-02, .0000000E+00,
     & .2213350E-03, .6824740E-02, .9480580E-02, .1000000E+01, .00,
     & .6743160E-04,-.1043770E-02, .4181700E-02, .0000000E+00,
     & .6743160E-04, .2986900E-02, .5971130E-01, .1000000E+01, .00,
     & .2513550E-04,-.5116570E-03, .2665640E-02, .0000000E+00,
     & .2513550E-04, .1506620E-02, .4071710E-01, .1000000E+01, .00,
     & .1081610E-04,-.3031020E-03, .2128620E-02, .0000000E+00,
     & .1081610E-04, .8824490E-03, .3250750E-01, .1000000E+01, .00,
     & .5145300E-05,-.1963160E-03, .1945940E-02, .0000000E+00,
     & .5145300E-05, .5384990E-03, .2543920E-01, .1000000E+01, .00,
     & .2841860E-05,-.1353600E-03, .1677610E-02, .0000000E+00,
     & .2841860E-05, .3634370E-03, .2103720E-01, .1000000E+01, .00,
     & .1730860E-05,-.9497230E-04, .1312660E-02, .0000000E+00,
     & .1730860E-05, .2612780E-03, .1794170E-01, .1000000E+01, .00/
      DATA (((TPAD(I,J,INU),J=1,4),I=1,2),TINF(INU),
     &     INU=177,184) /
     & .1088100E-05,-.6788460E-04, .1091560E-02, .0000000E+00,
     & .1088100E-05, .1874580E-03, .1458810E-01, .1000000E+01, .00,
     & .8613630E-06,-.5973650E-04, .1042190E-02, .0000000E+00,
     & .8613630E-06, .1637110E-03, .1418410E-01, .1000000E+01, .00,
     & .8369651E-06,-.6635050E-04, .1358990E-02, .0000000E+00,
     & .8369510E-06, .1671650E-03, .1522480E-01, .1000000E+01, .00,
     & .6827500E-06,-.5587210E-04, .1171480E-02, .0000000E+00,
     & .6827500E-06, .1441630E-03, .1389290E-01, .1000000E+01, .00,
     & .6094040E-06,-.5073110E-04, .1077010E-02, .0000000E+00,
     & .6094040E-06, .1327060E-03, .1319230E-01, .1000000E+01, .00,
     & .5963110E-06,-.4980280E-04, .1059790E-02, .0000000E+00,
     & .5963110E-06, .1306180E-03, .1306130E-01, .1000000E+01, .00,
     & .6392780E-06,-.5283910E-04, .1115970E-02, .0000000E+00,
     & .6392780E-06, .1374170E-03, .1348350E-01, .1000000E+01, .00,
     & .2025706E-05,-.2186878E-03, .7046864E-02, .0000000E+00,
     & .2025706E-05, .3637784E-03, .2791244E-01, .1000000E+01, .00/
      DATA (((TPAD(I,J,INU),J=1,4),I=1,2),TINF(INU),
     &     INU=185,192) /
     & .2449012E-05,-.2524742E-03, .7799482E-02, .0000000E+00,
     & .2449012E-05, .4153708E-03, .3000408E-01, .1000000E+01, .00,
     & .2853876E-05,-.2711736E-03, .7711191E-02, .0000000E+00,
     & .2853876E-05, .4538059E-03, .3086824E-01, .1000000E+01, .00,
     & .4073443E-05,-.3603662E-03, .9629610E-02, .0000000E+00,
     & .4073443E-05, .5851843E-03, .3565569E-01, .1000000E+01, .00,
     & .3651958E-05,-.2582784E-03, .5149791E-02, .0000000E+00,
     & .3651958E-05, .5035283E-03, .3061224E-01, .1000000E+01, .00,
     & .5319370E-05,-.3354652E-03, .5954901E-02, .0000000E+00,
     & .5319370E-05, .6505392E-03, .3502529E-01, .1000000E+01, .00,
     & .8492560E-05,-.4818871E-03, .7658930E-02, .0000000E+00,
     & .8492560E-05, .9090663E-03, .4252175E-01, .1000000E+01, .00,
     & .1377086E-04,-.7064469E-03, .1032753E-01, .0000000E+00,
     & .1377086E-04, .1279638E-02, .5146845E-01, .1000000E+01, .00,
     & .1461662E-04,-.5727074E-03, .6208960E-02, .0000000E+00,
     & .1461662E-04, .1226124E-02, .4608929E-01, .1000000E+01, .00/
      DATA (((TPAD(I,J,INU),J=1,4),I=1,2),TINF(INU),
     &     INU=193,200) /
     & .3197913E-04,-.1126462E-02, .1065666E-01, .0000000E+00,
     & .3197913E-04, .2253298E-02, .7019716E-01, .1000000E+01, .00,
     & .3530045E-04,-.9916705E-03, .7666453E-02, .0000000E+00,
     & .3530045E-04, .2168500E-02, .5976808E-01, .1000000E+01, .00,
     & .1671161E-03,-.4608833E-02, .3605752E-01, .0000000E+00,
     & .1671161E-03, .7712436E-02, .1500393E+00, .1000000E+01, .00,
     & .4092094E-03,-.9497234E-02, .6203619E-01, .0000000E+00,
     & .4092094E-03, .1541597E-01, .2417117E+00, .1000000E+01, .00,
     & .2434950E-03,-.3928087E-02, .1790729E-01, .0000000E+00,
     & .2434950E-03, .8092812E-02, .1193843E+00, .1000000E+01, .00,
     & .6053093E-03,-.8333566E-02, .3339577E-01, .0000000E+00,
     & .6053093E-03, .1555924E-01, .1739616E+00, .1000000E+01, .00,
     & .8507600E-03,-.8225053E-02, .2199495E-01, .0000000E+00,
     & .8507600E-03, .1817275E-01, .1740635E+00, .1000000E+01, .00,
     & .1512928E-02,-.1089150E-01, .2075969E-01, .0000000E+00,
     & .1512928E-02, .2598628E-01, .2027160E+00, .1000000E+01, .00/
      DATA (((TPAD(I,J,INU),J=1,4),I=1,2),TINF(INU),
     &     INU=201,208) /
     & .5101098E-02,-.3665276E-01, .7587805E-01, .0000000E+00,
     & .5101098E-02, .6629066E-01, .3723330E+00, .1000000E+01, .00,
     & .4653500E-02,-.2260518E-01, .2839950E-01, .0000000E+00,
     & .4653500E-02, .5490622E-01, .2942551E+00, .1000000E+01, .00,
     & .1106198E-01,-.4818140E-01, .5884696E-01, .0000000E+00,
     & .1106198E-01, .1017552E+00, .4172756E+00, .1000000E+01, .00,
     & .1600881E-01,-.5483757E-01, .5127584E-01, .0000000E+00,
     & .1600881E-01, .1251087E+00, .4423680E+00, .1000000E+01, .00,
     & .1928267E-01,-.5280840E-01, .3751520E-01, .0000000E+00,
     & .1928267E-01, .1362609E+00, .4442577E+00, .1000000E+01, .00,
     & .2615498E-01,-.6473146E-01, .4075034E-01, .0000000E+00,
     & .2615498E-01, .1676252E+00, .4946038E+00, .1000000E+01, .00,
     & .3727908E-01,-.9137727E-01, .5735914E-01, .0000000E+00,
     & .3727908E-01, .2226437E+00, .6093447E+00, .1000000E+01, .00,
     & .3916857E-01,-.9998208E-01, .6590945E-01, .0000000E+00,
     & .3916857E-01, .2319352E+00, .6238246E+00, .1000000E+01, .00/

      DO K=105,208
         TO3INF(K)=TINF(K)
         DO  J=1,4
            PADO3(1,J,K)=TPAD(1,J,K)
            PADO3(2,J,K)=TPAD(2,J,K)
         ENDDO
      ENDDO
