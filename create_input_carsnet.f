...
c      INTEGER LOZ,LWV
      ...
      CHARACTER HEADER*3,CDATE*6,FILER*100
      CHARACTER FILEB*14,FILEL*100,CK*10,FILOUT*100,FILTAG*100
...
c	the index of input data file	
	CHARACTER FINDEX*10
...
c	FDAT='H:\AERONET_INVERSION\data\'	!!! from command line arguments
	call getarg(7,FDAT)
	call getarg(8,FINDEX)
	OPEN(250,
     &FILE=TRIM(FOUT)//'FNAME.TXT.'//TRIM(FINDEX),STATUS='UNKNOWN')

c	FILER=TRIM(FDAT)//TRIM(STNS_FN)//'\'//TRIM(FDATA)//'.ALR' !!!
c	FILEL=TRIM(FDAT)//TRIM(STNS_FN)//'\'//TRIM(FDATA)//'.ALL' !!!
	FILER=TRIM(FDAT)//'\'//TRIM(FDATA)//'.ALR.'//TRIM(FINDEX) !!!
	FILEL=TRIM(FDAT)//'\'//TRIM(FDATA)//'.ALL.'//TRIM(FINDEX) !!!

c	生成FNAME
      OPEN(UNIT=20,
     &FILE=TRIM(FOUT)//'FNAME.'//TRIM(FINDEX),STATUS='UNKNOWN')
...
C	TO READ OZONE DATA
	OPEN(120,
     &FILE=TRIM(FIPT)//'METEO_'//TRIM(STNS_ID)//'.DAT',STATUS='OLD')
	DO I=1,6
		READ(120,*)
	ENDDO
	NOZONE=0
...
!     Reads date to be processed
      OPEN(UNIT=30,FILE=TRIM(FOUT)//'FNAME.'//TRIM(FINDEX),STATUS='OLD')
