...
stns_fn='wuhan-both';
stns_id='994';
fout=['f:\CARSNET_INVERSION\CIMEL_NETWORK\' stns_fn '\dubovik\'];
mkdir(fout);
fpath=['f:\CARSNET_INVERSION\AERONET_INVERSION\output\' stns_fn '\'];
...
fidw=fopen([fout 'Dubovik_stats_' stns_fn '_' stns_id '_20130530.dat'],'w');
...
ind0=find(stats_inversion(:,1)>=2012 & stats_inversion(:,7) >= 0.4 & stats_inversion(:,26) > 0.63); %AOD at 440 > 0.4 and SSA 675 > 0.63
...
subplot(3,1,2);
linec=['-c';'-m';'-r';'-g';'-b';'-k';'-y'];
ind=find(cmonth(:,1)>=2012);
plot(datenum(cmonth(ind,1:6)),cmonth(ind,23:26),'-*','markersize',4);hold on;
datetick('x',2);hold on;
set(gca,'xtick',[datenum(2014,1,1) datenum(2014,4,1) datenum(2014,7,1) datenum(2014,10,1) datenum(2015,1,1) datenum(2013,5,1)] ,...
...
figure;
subplot(4,1,4);
ind=find(stats_inversion(:,1)>=2013 & stats_inversion(:,7) >= 0.4 & stats_inversion(:,26) > 0.63); %AOD at 440 > 0.4 and SSA 675 > 0.63
plot(datenum(stats_inversion(ind,1:6)),stats_inversion(ind,23:26),'-*');hold on;
for im=1:12;
    plot([datenum(2012,im,30),datenum(2012,im,30)],[0.6 1],'-r');
end;
datetick('x',12);hold on;
set(gca,'xlim',[datenum(stats_inversion(ind(1),1:6))-5 datenum(stats_inversion(end,1:6))+5]);
set(gca,'xtick',[datenum(2012,1,1) datenum(2012,2,1) datenum(2012,4,1) datenum(2012,6,1) datenum(2012,8,1) datenum(2012,10,1) datenum(2012,12,1)],...
    'xticklabe',{'Jan12','Feb12','Apr12','Jun12','Aug12','Oct12','Dec12'});
set(gca,'xminortick','on','yminortick','on');
...
ind=find(stats_inversion(:,1)==2012);
